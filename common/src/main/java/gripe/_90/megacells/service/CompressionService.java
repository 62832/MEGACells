...
public class CompressionService {
    ...
    public void loadRecipes(RecipeManager recipeManager) {
        ...
        var candidates = Stream.concat(
                        ...
                        allRecipes.stream().filter(this::isDecompressionRecipe))
                .toList();
        ...
        var validRecipes = candidates.stream()
                .filter(recipe -> {
                    ...
                    var checkAgainst = candidates.stream()
                            .filter(
                                    isCompressionRecipe(recipe)
                                            ? this::isDecompressionRecipe
                                            : this::isCompressionRecipe)
                            .toList();
        ...
        var decompressed =
                validRecipes.stream().filter(this::isDecompressionRecipe).toList();
