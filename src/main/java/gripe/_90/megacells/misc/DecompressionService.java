...
public class DecompressionService implements IGridService, IGridServiceProvider, ICraftingProvider {
    public static final String TAG_PATTERN_PRIORITY = "dcp";
    ...
    public void onServerStartTick() {
        if (!patternOutputs.isEmpty()) {
            for (var output : Object2LongMaps.fastIterable(patternOutputs)) {
                ...
                if (inserted >= amount) {
                    patternOutputs.removeLong(what);
                } else if (inserted > 0) {
    ...
    @Override
    public boolean pushPattern(IPatternDetails details, KeyCounter[] keyCounters) {
